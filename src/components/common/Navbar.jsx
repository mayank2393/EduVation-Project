...
const Navbar = () => {
  ...
  const [subLinks, setSubLinks] = useState([]); // catalogs ke links ko store karne ke liye
        ...
        <div className="flex gap-x-4 items-center">
          {user && user?.accountType != "Instructor" && (
            <Link to="/dashboard/cart" className="relative">
              ...
              {totalItems > 0 && (
                <span>
                  {totalItems} // directly accessed from redux store and got via
                  useSelector hook jaisa uper kiya hai
                </span>
              )}
            </Link>
          )}
